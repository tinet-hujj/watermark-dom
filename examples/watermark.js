(function (root, factory) {
  ...
  var loadMark = function (settings) {
    ...
    let gotoMinotorIndex = 0;

    const gotoMinotor = () => {
      gotoMinotorIndex++;
      if (gotoMinotorIndex < document.getElementById(defaultSettings.watermark_id).childNodes.length) {
        return;
      }
      console.log('执行')
    ...
    document.getElementById(defaultSettings.watermark_id).childNodes.forEach((node, i) => {
      html2canvas(node).then((canvas) => {
        canvas.style.position = node.style.position;
        canvas.style.left = node.style.left;
        canvas.style.top = node.style.top;
        canvas.style.zIndex = -999999;
        document.getElementById(defaultSettings.watermark_id).replaceChild(canvas, node);
        gotoMinotor();
      });
    })
  ...
  const MutationObserverIn = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver;
  var watermarkDom = new MutationObserverIn(callback);
  var option = {
    'childList': true,
    'attributes': true,
    'subtree': true,
